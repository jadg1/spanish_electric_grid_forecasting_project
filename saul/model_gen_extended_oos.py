...
df.dropna(subset=['gen_total'], inplace=True)

# 2) Definir horas para hold-out (out of sample)
holdout_hours = 24  # horas out of sample
cutoff = df['datetime'].max() - pd.Timedelta(hours=holdout_hours)

# 3) Dividir en entrenamiento (train) y hold-out (predicción final)
train_df = df[df['datetime'] <= cutoff].copy()
holdout_df = df[df['datetime'] > cutoff].copy()

# 4) Definir características (X) y target (y) usando *solo* train_df
feature_cols = [c for c in df.columns if c not in ['datetime','gen_total']]
X = train_df[feature_cols]
y = train_df['gen_total']

# 5) Validación interna con TimeSeriesSplit en la parte de entrenamiento
...
with pd.ExcelWriter('cross_validation_metrics_generacion.xlsx') as writer:
...
# 7) Predicción out of sample (hold-out)
X_holdout = holdout_df[feature_cols]
y_holdout = holdout_df['gen_total']
...
xgb_metrics = {
    ...
    'RMSE': sqrt(mean_squared_error(y_holdout, xgb_preds_holdout)),
...
dt_metrics = {
    ...
    'RMSE': sqrt(mean_squared_error(y_holdout, dt_preds_holdout)),
...
rf_metrics = {
    ...
    'RMSE': sqrt(mean_squared_error(y_holdout, rf_preds_holdout)),
...
metrics_df.to_excel('holdout_metrics_generacion.xlsx', index=False)

print("\nMétricas Holdout (Out of Sample):")
print(metrics_df)

# 9) Graficar las predicciones en el hold-out
...
plt.title(f"Total Generation - Last {holdout_hours} hours (Out of Sample)")
plt.legend()
plt.tight_layout()
plt.savefig('gen_oos_predictions.png')
plt.show()