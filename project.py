...
generate_forecast_plots(
    ...
    ml_predictions_dict,
    cv_results_dict,
    y_holdout_arima_dict,
    arima_predictions_dict
)

def calculate_and_display_gen_demand_diff(df):
    """
    Calculate and display the difference between generation and demand.
    """
    df['generation_demand_diff'] = df['gen_total'] - df['demand']
    print(df[['gen_total', 'demand', 'generation_demand_diff']].head())
    return df

df = calculate_and_display_gen_demand_diff(df)

def calculate_and_plot_gen_demand_diff(df, title="Generation vs. Demand Difference"):
    """
    Calculates generation-demand difference and plots it.
    """
    df['generation_demand_diff'] = df['gen_total'] - df['demand']
    ...
    # Display the first few rows for quick verification
    print(df[['gen_total', 'demand', 'generation_demand_diff']].head())

calculate_and_display_gen_demand_diff(df)
...
def detect_and_plot_anomalies(filepath, date_col='Date', value_col='price', contamination=0.05):
    """
    Loads data, detects anomalies using Isolation Forest, and plots the results.

    Parameters:
        filepath (str): Path to the Excel file.
        date_col (str): Name of the datetime column.
        value_col (str): Column name for the data to analyze.
    ...
    df["anomaly"] = iso_forest.fit_predict(X=df[["Value"]])
    ...
    # Plot
    plt.figure(figsize=(12, 6))
    plt.plot(normal["Date"], normal["Value"], color="blue", marker='o', linestyle='-', label="Normal Data")
    plt.scatter(anomalies["Date"], anomalies["Value"], color="red", label="Anomalies")

    plt.title("Data with Anomaly Detection")
    plt.xlabel("Date")
    plt.ylabel("Value")
    plt.legend()
    plt.grid(True)
...
# Call the anomaly detection function using the clearly defined file path
detect_and_plot_anomalies(
    filepath=gas_filtrado_file, 
    date_col='Date', 
    value_col='price', 
    contamination=0.05
)
